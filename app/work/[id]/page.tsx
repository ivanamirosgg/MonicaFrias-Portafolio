  ...
  return (
    <main className="min-h-screen bg-[#F3F4F4] text-[#262626] selection:bg-[#262626] selection:text-white">
      ...
      <div className="pt-32 md:pt-48 pb-20 px-6 md:px-16 max-w-[1800px] mx-auto">
        ...
        {(project.aboutText && project.aboutImage) && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-32">
            {project.gallery.map((item: any, index: number) => {
              ...
              if (item.type === "text-image") {
                // Buscamos si es el texto exacto que quieres forzar
                const isTargetText = item.text && item.text.includes("El proceso del fashion film");

                if (isTargetText) {
                  // FORZADO ESTRICTO: IMAGEN A LA IZQUIERDA Y TEXTO A LA DERECHA
                  return (
                    <div key={index} className="md:col-span-2 flex flex-col md:flex-row gap-12 lg:gap-24 my-16 items-center">
                      
                      {/* IMAGEN: Fijada permanentemente a la izquierda (order-1) */}
                      <div className="w-full md:w-1/2 order-1 md:order-1">
                        {/* Se cambia 'object-cover' por 'object-contain' para que no se corte */}
                      ...
                      {/* TEXTO: Fijado permanentemente a la derecha (order-2) */}
                      <div className="w-full md:w-1/2 order-2 md:order-2">
                        <p 
                          className={`${manrope.className} text-sm md:text-base leading-relaxed text-[#262626] opacity-80 uppercase tracking-widest font-bold text-justify`}
                ...
                // COMPORTAMIENTO NORMAL PARA CUALQUIER OTRO TEXTO-IMAGEN (Respeta data.ts)
              ...
              if (item.type === "video") {
                return (
                  <div key={index} className="md:col-span-2 my-12">
                    ...
                    {item.text && (
                      <p className={`${manrope.className} mt-6 text-xs md:text-sm uppercase tracking-[0.2em] opacity-60 text-center font-bold`}>
                        {item.text}
                      </p>
                    )}
                  </div>
              ...
              // =========================================================
              // LÓGICA PARA OUBAITORI (ID: 1)
              // =========================================================
              if (currentId === 1 && item.size === "large") {
                return (
              ...
              if (currentId === 4 || currentId === 3) {
                    ...
                    <div className={`relative w-full ${!isFullWidth ? 'aspect-[3/4]' : ''} overflow-hidden bg-gray-200 rounded-[2rem] shadow-sm`}>
                      {isFullWidth ? (
                        ...
                        <Image 
                          ...
                          alt={item.text || "Proyecto"} 
                          fill 
                          className="object-cover hover:scale-105 transition-transform duration-700 block" 
                          unoptimized={true} 
                        />
                      )}
                    </div>
                ...
                if (index === 0) {
                  return (<div key={index} className="md:col-span-2"><FlipCard src={item.src} text={item.text} isLarge={true} /></div>);
                }
                if (item.size === "triptych") {
                    const isFirstOfTriptych = project.gallery[index-1]?.size !== "triptych";
                    if (isFirstOfTriptych) {
                        return (
                            <div key={index} className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 mt-12 mb-12">
                                ...
                                {project.gallery[index+1] && (
                                    <div className="flex flex-col gap-4">
                                        <div className="relative w-full aspect-[9/16] overflow-hidden bg-gray-200">
                                            <Image src={project.gallery[index+1].src} alt={project.gallery[index+1].text || "Proyecto"} fill className="object-cover hover:scale-105 transition-transform duration-700" unoptimized={true} />
                                        </div>
                                    </div>
                                )}
                                {project.gallery[index+2] && (
                                    <div className="flex flex-col gap-4">
                                        <div className="relative w-full aspect-[9/16] overflow-hidden bg-gray-200">
                                            <Image src={project.gallery[index+2].src} alt={project.gallery[index+2].text || "Proyecto"} fill className="object-cover hover:scale-105 transition-transform duration-700" unoptimized={true} />
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <div key={index} className="md:col-span-2 mb-8">
                       <div className="flex flex-col gap-4">
                           <div className="relative w-full aspect-[21/9] overflow-hidden bg-gray-200">
                               <Image src={item.src} alt={item.text || "Proyecto"} fill className="object-cover object-[center_30%] hover:scale-105 transition-transform duration-700" unoptimized={true} />
                           </div>
                       </div>
                    </div>
                  );
                }
                return (<div key={index} className="col-span-1"><FlipCard src={item.src} text={item.text} isLarge={false} /></div>);
              }

              // =========================================================
              // LÓGICA MÉXICO EN LA PIEL (ID: 2) -> NUEVO DISEÑO
              // =========================================================
              else if (currentId === 2) {
               ...
               if (index === 0) {
                  return (
                    <div key={index} className="md:col-span-2 flex flex-col gap-8 my-12 lg:my-16">
                      <p className={`${manrope.className} text-sm md:text-base leading-relaxed text-[#262626] opacity-80 uppercase tracking-widest font-bold text-justify`}>
                        Inspirada en el azul maya —pigmento histórico que simboliza el cielo, el agua y la vida— la colección reinterpreta la cosmovisión mesoamericana para el contexto del streetwear contemporáneo. la identidad visual se articula a través del ollín (movimiento), un logotipo que representa el dinamismo de las culturas urbanas juveniles y la continuidad de nuestras tradiciones en el mundo moderno. Promover la herencia cultural en la moda es una prioridad creativa: un ejercicio que no solo fortalece la identidad propia, sino que permite que el diseño mexicano dialogue con el mercado global desde una posición de autenticidad, innovación y respeto por la historia.
                      </p>
                      <div className="relative w-full mt-4 rounded-[2rem] overflow-hidden bg-gray-200 shadow-sm">
                        <Image 
                          src="/MEXICOOO.png" 
                          alt="Moodboard México en la piel" 
                          width={0} height={0} sizes="100vw" style={{ width: '100%', height: 'auto' }} 
                      ...
                      <div className="relative w-full rounded-[2rem] overflow-hidden bg-gray-200 shadow-sm">
                        <Image 
                          src="/azulino.png" 
                          alt="Proceso de diseño colaborativo" 
                          width={0} height={0} sizes="100vw" style={{ width: '100%', height: 'auto' }} 
                          className="block hover:scale-105 transition-transform duration-700" 
                          unoptimized={true} 
                        />
                ...
                if (index === 3 && project.gallery.length >= 7) {
                  return (
                    <div key={index} className="md:col-span-2 grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 mt-12 mb-12">
                      <div className="lg:col-span-1">
                        <div className="relative w-full h-[60vh] lg:h-full rounded-[2rem] overflow-hidden bg-gray-200 shadow-sm">
                          <Image src={project.gallery[3].src} alt={project.gallery[3].text || "Proyecto"} fill className="object-cover hover:scale-105 transition-transform duration-700" unoptimized={true} />
                        </div>
                      </div>
                      <div className="lg:col-span-2 flex flex-col gap-4 md:gap-8">
                        ...
                        <div className="relative w-full aspect-[21/9] lg:h-full min-h-[30vh] rounded-[2rem] overflow-hidden bg-gray-200 shadow-sm flex-grow">
              ...
              // =========================================================
              // LÓGICA NEW ROMANTICS (ID: 5) -> PRIMERA IMAGEN ESTILO BORDEADO
              // =========================================================
        ...
        <div className="border-t border-[#262626]/20 pt-12 md:pt-24 flex flex-col items-center">
            <p className={`${manrope.className} text-xs text-[#262626] opacity-60 uppercase tracking-[0.3em] mb-8`}>
                Next Project
            </p>
            <Link href={`/work/${nextProject.id}`} className="group flex flex-col items-center text-center gap-6 cursor-pointer">
                <h2 className={`${krona.className} text-4xl md:text-7xl group-hover:opacity-50 transition-opacity duration-500 uppercase`}>
                    {nextProject.title}
                </h2>
                <div className="relative w-64 h-40 md:w-96 md:h-56 rounded-[2rem] overflow-hidden shadow-xl transition-all duration-500">
                    <Image 
                      src={nextProject.src} 
                      alt="Next Project" 
                      fill 
